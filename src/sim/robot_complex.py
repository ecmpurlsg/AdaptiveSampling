...
class Robot:
    def __init__(self, position: Tuple[float, float], radius: float, corridor_bounds: dict, door_position: Tuple[float, float]):
        ...
        self.door_position = door_position  # Keep for backwards compatibility
        self.all_doors = []  # Will be set by Simulation to store all door positions
        self.people = None
    ...
    def set_goal(self, goal: Tuple[float, float]):
        ...
        # Store the actual goal (final destination) for 10m projection (only if not already set)
        if not hasattr(self, 'actual_goal') or self.actual_goal is None:
            self.actual_goal = np.array(goal, dtype=float)
    
    def get_projected_goal(self) -> np.ndarray:
        """Compute the projected goal based on doors ahead of the robot (same as neural network).
        ...
        if not hasattr(self, 'last_passed_door_idx'):
            self.last_passed_door_idx = -1
        
        # Get door x positions from all_doors (list of (x, y) tuples) or door_x_positions
        door_x_positions = []
        if hasattr(self, 'door_x_positions') and self.door_x_positions:
            # Use stored door x positions
            door_x_positions = self.door_x_positions
        elif hasattr(self, 'all_doors') and self.all_doors:
            # Extract x positions from all_doors
            door_x_positions = [door_pos[0] for door_pos in self.all_doors]
        
        if not door_x_positions:
            # No doors available, use actual goal
            return np.array(actual_goal, dtype=float)
        
        # Update last_passed_door_idx: check if robot has passed the middle of any door
        for i, door_x in enumerate(door_x_positions):
            door_middle_x = door_x  # Middle of door is at door_x
            if robot_x > door_middle_x and i > self.last_passed_door_idx:
                self.last_passed_door_idx = i
        
        # Find doors ahead of the last passed door
        doors_ahead = []
        for i, door_x in enumerate(door_x_positions):
            if i > self.last_passed_door_idx:
        ...
        if len(doors_ahead) >= 2:
            ...
            if hasattr(self, 'start_position') and self.start_position is not None:
    ...
    def get_closest_door_x(self) -> float:
        """Get the x-coordinate of the next door (not yet passed) to the robot.
        
        If the robot has passed a door's x-location, it considers the next door.
        If all doors are passed, returns the last door's x-coordinate.
        """
        robot_x = self.position[0]
        
        if hasattr(self, 'door_x_positions') and self.door_x_positions:
            # Find the next door that the robot hasn't passed yet
            for door_x in self.door_x_positions:
                if robot_x < door_x:  # Robot hasn't passed this door yet
                    return float(door_x)
            # If all doors are passed, use the last door
            return float(self.door_x_positions[-1])
        elif hasattr(self, 'all_doors') and self.all_doors:
            # Fallback: extract x positions and find next door
            door_x_positions = [door_pos[0] for door_pos in self.all_doors]
            for door_x in door_x_positions:
                if robot_x < door_x:  # Robot hasn't passed this door yet
                    return float(door_x)
            # If all doors are passed, use the last door
            return float(door_x_positions[-1])
        elif hasattr(self, 'door_position'):
            # Fallback to single door
            return float(self.door_position[0])
        else:
            # No door available, return a default value
            return 0.0
    
    def _allow_wall_escape(self, dt: float):
        """Allow movement away from walls if robot is touching them.
        
        When the robot is touching a wall:
        1. If velocity points away from wall, allow that movement
        2. If velocity is zero or parallel to wall (no component away), apply escape velocity
        """
        ...
        # Calculate proposed new position from velocity
        proposed_pos = pos + vel * dt
        
        # Escape velocity magnitude (small push away from wall)
        escape_speed = 0.15  # 0.15 m/s escape velocity
        
        # Check each wall and allow movement away from it
        # Left wall (x_min)
        dist_to_left = pos[0] - bounds['x_min'] - radius
        if dist_to_left <= 0.01:  # Touching or very close to left wall
            if vel[0] > 0.01:  # Moving right (away from left wall) with significant component
                # Allow movement away from wall (ensure we don't go into wall)
                self.position[0] = max(bounds['x_min'] + radius, proposed_pos[0])
            elif vel[0] <= 0.01:  # Velocity is zero or pointing into/parallel to wall
                # Apply escape velocity away from wall
                escape_vel = np.array([escape_speed, 0.0])
                escape_pos = pos + escape_vel * dt
                self.position[0] = max(bounds['x_min'] + radius, escape_pos[0])
                # Update velocity to reflect escape movement (preserve y-component if exists)
                self.velocity[0] = escape_speed
        
        # Right wall (x_max)
        dist_to_right = bounds['x_max'] - pos[0] - radius
        if dist_to_right <= 0.01:  # Touching or very close to right wall
            if vel[0] < -0.01:  # Moving left (away from right wall) with significant component
                # Allow movement away from wall
                self.position[0] = min(bounds['x_max'] - radius, proposed_pos[0])
            elif vel[0] >= -0.01:  # Velocity is zero or pointing into/parallel to wall
                # Apply escape velocity away from wall
                escape_vel = np.array([-escape_speed, 0.0])
                escape_pos = pos + escape_vel * dt
                self.position[0] = min(bounds['x_max'] - radius, escape_pos[0])
                # Update velocity to reflect escape movement (preserve y-component if exists)
                self.velocity[0] = -escape_speed
        
        # Bottom wall (y_min)
        dist_to_bottom = pos[1] - bounds['y_min'] - radius
        if dist_to_bottom <= 0.01:  # Touching or very close to bottom wall
            if vel[1] > 0.01:  # Moving up (away from bottom wall) with significant component
                # Allow movement away from wall
                self.position[1] = max(bounds['y_min'] + radius, proposed_pos[1])
            elif vel[1] <= 0.01:  # Velocity is zero or pointing into/parallel to wall
                # Apply escape velocity away from wall
                escape_vel = np.array([0.0, escape_speed])
                escape_pos = pos + escape_vel * dt
                self.position[1] = max(bounds['y_min'] + radius, escape_pos[1])
                # Update velocity to reflect escape movement (preserve x-component if exists)
                self.velocity[1] = escape_speed
        
        # Top wall (y_max)
        dist_to_top = bounds['y_max'] - pos[1] - radius
        if dist_to_top <= 0.01:  # Touching or very close to top wall
            if vel[1] < -0.01:  # Moving down (away from top wall) with significant component
                # Allow movement away from wall
                self.position[1] = min(bounds['y_max'] - radius, proposed_pos[1])
            elif vel[1] >= -0.01:  # Velocity is zero or pointing into/parallel to wall
                # Apply escape velocity away from wall
                escape_vel = np.array([0.0, -escape_speed])
                escape_pos = pos + escape_vel * dt
                self.position[1] = min(bounds['y_max'] - radius, escape_pos[1])
                # Update velocity to reflect escape movement (preserve x-component if exists)
                self.velocity[1] = -escape_speed
    ...
    def add_gaussian_bump_to_path(self, alpha: float, sigma: float = 1.5, 
        ...
        # Get the closest door's x-coordinate for the bump location
        door_x = self.get_closest_door_x()
    ...
    def update(self, dt: float, people: List[Person]):
        ...
        # Update goal to match the neural network's projected goal (door-based)
        planner_goal = self.get_projected_goal()
        # Only update if the goal has changed significantly (avoid constant updates)
        if np.linalg.norm(planner_goal - self.goal) > 0.1:  # 10cm threshold
            self.goal = planner_goal
            self.nav.set_goal(tuple(planner_goal))
            # Note: We don't recompute global_path here to preserve any modifications (e.g., Gaussian bumps)
            # The global_path will be updated when needed (e.g., when goal changes significantly)
    ...
    def get_egocentric_costmap(self, size=4.0, resolution=0.1, inflation_radius=0.2):
        ...
        # Use closest door for costmap
        if hasattr(self, 'all_doors') and self.all_doors and hasattr(self, 'corridor_bounds'):
            # Find closest door
            min_dist = float('inf')
            closest_door_pos = self.all_doors[0]
            for door_pos in self.all_doors:
                dist = np.linalg.norm(np.array(door_pos) - self.position)
                if dist < min_dist:
                    min_dist = dist
                    closest_door_pos = door_pos
            door_world = np.array(closest_door_pos, dtype=float)
        elif hasattr(self, 'door_position') and hasattr(self, 'corridor_bounds'):
            # Fallback to single door
    ...
    def get_navigation_info(self, lookahead_distance: float = 2.0):
        ...
        # Use closest door
        if hasattr(self, 'all_doors') and self.all_doors:
            min_dist = float('inf')
            closest_door_pos = self.all_doors[0]
            for door_pos in self.all_doors:
                dist = np.linalg.norm(np.array(door_pos) - self.position)
                if dist < min_dist:
                    min_dist = dist
                    closest_door_pos = door_pos
            door_world = np.array(closest_door_pos)
        else:
